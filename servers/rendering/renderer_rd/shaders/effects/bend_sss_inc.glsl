...
// Helper functions
int bend_min(const int a, const int b) { return a > b ? b : a; }
int bend_max(const int a, const int b) { return a > b ? a : b; }
...
DataList BuildData(vec4 inLightProjection, ivec2 inViewportSize, ivec2 inMinRenderBounds, ivec2 inMaxRenderBounds, bool inExpandedZRange = false, int inWaveSize = 64)
{
    ...
    for (int q = 0; q < 4; q++) {
        ...
        if ((bounds.z - bounds.x) > 0 && (bounds.w - bounds.y) > 0) {
            ...
            if (q == 1) axis_delta = biased_bounds.z + biased_bounds.y;
            if (q == 2) axis_delta = -biased_bounds.x - biased_bounds.w;
            if (q == 3) axis_delta = -biased_bounds.z + biased_bounds.w;
